...
def index(request):
    if not request.user.is_authenticated:
        return render(
            request,
            "login.html",
            {"message": "You must be logged in.", "return_to": "index"},
        )
...
def signup(request):
    if request.method == "POST":
        ...
        if password != confirmation:
            return render(request, "signup.html", {"alert": "Passwords do not match."})
...
def settings(request):
    if not request.user.is_authenticated:
        return render(
            request,
            "login.html",
            {"message": "Log in to view your settings.", "return_to": "settings"},
        )
    ...
    return render(request, "settings.html", {"user": request.user, "key": user.key})
...
def uploader(request):
    if not request.user.is_authenticated:
        return render(
            request,
            "login.html",
            {"message": "Log in to begin uploading", "return_to": "uploader"},
        )
    if request.method == "POST":
        ...
        # Handle pasted text
            ...
            submit_text = re.sub(
                r"\r", "", submit_text
            )  # eliminate carriage returns submitted through HMTL
        ...
        if result == "key invalid":
            return HttpResponse(
                "Invalid OpenAI key. Make sure to set your key in Settings."
            )
        if result[0] == "":
            result = result[1:]  # eliminate first line if it is blank
        title = get_title(result)
        save_in_archive = Archive(
            user=request.user,
            title=title,
...
def workshop(request):
    ...
    return render(request, "workshop.html", {"articles": articles})
...
def workshop_render(request, id):
    ...
    if unedited_text == "":
        ...
        )  # if no final text exists yet, grab the original text
    ...
    return render(
        request, "workshop_render.html", {"text": preview_text, "article_id": id}
    )
...
def workshop_download(request, id):
    ...
    return render(
        request,
        "workshop_download.html",
        {"article_id": id, "text": final_text_string.split("\n")},
    )
